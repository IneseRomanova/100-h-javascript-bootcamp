...
console.log("Hello, " + name + "!") // nevajag
}

personalGreeting ("Johny")
personalGreeting ("Grald")
...
function myFunction(currentYear, yearOfBirth) {
...
/*
    getArea: function(){ //method
        return 2 * 3.14 * this.radius //this lieto, ja funkcijas iekšiene
    } */
    /* getColor: function() {
      return this.color
    }
}
     /*circle.color ===circle.getColor() === Circle("color")*/
 /*console.log(circle.getArea().toFixed(2)) */ 

 
 //toFixed(2) cik punktus jārāda aiz kmata
// console.log(circle.getArea()) // 62.8
...
function startsWithSummer(text) {
    ...
    let fisrtSixLetters = text.slice(0, 6) 
    return fisrtSixLetters === 'Summer'
}
...
return (first >= 13 && first <=19) ||(second >=13 && second <=19) || (third >= 12 && third <=19 )
}
...
function mixStart(s) {
   let firstTwoCharacters = s.slice (1,3)  //s[1] + s[2]
...
function commonEnd(firstArr, secondArr) {
  let i = firstArr.length
  let j = secondArr.lenght
 return firstArr[0] === secondArr[0] || first[i] === secondArr[j]
}
...
for (let i = 0; i ,10; i++){
  ...
  console.log(i)
  if (i===5) {
    break    //ja velas lai loop parstaj ja i = 5
  }
}
*/
/*
...
let otherText = "ABC-DEF-AGHG-HVHJM-BJB"
//console.log(otherText[0])
let otherTextSplit = otherText.split("-")
//console.log(otherTextSplit[3])

//Modulo operator
//%

let z = 5 % 2 // 1  cik x 2 ietilpst 5, paliek pari 1
let y = 13 % 5 // 5 ieiet 2 reizes un paliek pari 3
let w = 15 % 5 //4
let o = 15 % 5 //0
let p = 16 % 5 //1
let k = 17 % 5 //2
...
//alert('warning') //izlec broser paziņojums
/*let age = prompt ('How old are you?') //user can put some text or number
//console.log(age)
...
if (age < 21){
  console.log("You cannot enter the control")
}else{
  console.log('Time to gamble')
  enteredTheCasino = true
}
if (enteredTheCasino){
  let machineChoice = prompt("Do you want to use the betting machine (B) or rulette (R)?")
  if (machineChoice === 'B'){

  alert("You have chosen to play on the betting machines!")
