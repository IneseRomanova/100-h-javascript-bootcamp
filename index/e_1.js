...
/**
 * Return true if the person is allowed to gamble 
 ...
 *  1) 'Back to the past!' , 2) 'Staying in the present!' 
 ...
 */
 ...
 //console.log(compareStringsIgnoreCase('hallo', 'Hello!'))          // false



/**
 * Return the word that has more characters in it.
 * @param {*} word string
 * @param {*} otherWord string
 */
/*function chooseLongestWord(word, otherWord) {
 if (word.length === otherWord.length || word.length > otherWord.length)
     return word
 } else {
    return otherWord
 }
...
function addressFormally(fullName, isMarried, isFemale) {
    ...
    if (isFemale) {
        ...
        let name = ""
        let nameSplit = fullName.split(" ") //This is an array
        name = nameSplit[0] 
        
        let lastName = nameSplit[1]
        let firstCharacterOfLastName = lastName[0]

        return firstName + name + " " + firstCharacterOfLastName
    }
...
/**
 * Return the average of provided ages. 
 * If the age is negative, do not consider it in calculations.
 * @param {*} ages array of numbers 
 */
function averageAge(ages) {
    let sum = 0
    let negativeCount = 0
    for (age of ages) {    //for every age in ages
        if (age >= 0) {
           sum += age 
        } else {
            negativeCount ++
...
function getOddNumbers(numbers) {
    ...
    //modulo operator
    for (nr of numbers){
       if (nr % 2 !== 0){ // The number is even
            oddNumbers.push(nr)    //pievieno kopao
       }
    }
   return oddNumbers
}
 ...
 * @param {*} text string
 */
function reverseText(text) { //jaievieto katrs burts no beigam
    let reversed = ""
    for(let i = text.length - 1; i >= 0; i--){//i-- = i-1
      reversed += text1[i] //
    }  
     return reversed
}
 //onsole.log(reverseText("ABC")) // 'CBA'
