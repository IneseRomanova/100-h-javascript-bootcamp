...
function makeMove(col, row){
 ...
 function isThereAnEmptyCell(){
     for (let row = 0; row<gameBoard.lenght; row++){
     let currentRow = gameBoard[row]
       for (let col = 0; col < currentRow.lenght; col ++){
           let tile = gameBoard[row][col]
           if (tile === EMPTY_TILE_SYMBOL){
               return true
           }
    ...
    let enemyValue = -1
     ...
     for(let row=0; row < gameBoard.length; row++){
         ...
         let rowSum = 0
           ...
           if (rowSum === playerValue* CELLS_HORIZONTALLY || rowSum === playerValue* CELLS_HORIZONTALLY ){
               return true
           }
     }
     return false
